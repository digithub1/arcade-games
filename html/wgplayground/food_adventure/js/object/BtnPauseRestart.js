...
game.object.BtnPauseRestart = me.GUI_Object.extend({
	init: function(x, y, settings = {}){
		settings.texture = "UI";
		settings.image = game.textureMap.get(settings.texture);
		game.util.__populateAtlasIndices([
			"button_text_replay","button_text_replay_h"
		], settings);
		settings.framewidth = settings.framewidth || 100;
		settings.frameheight = settings.frameheight || 34;
		...
		this.addAnimation('idle', [{ name: "button_text_replay", delay: 100 }]);
		this.addAnimation('select', [{ name: "button_text_replay_h", delay: 100 }]);
    ...
    onRelease : function(event){
        if(this.click){
            ...
            me.timer.setTimeout(()=>{
                if(once){
                    once = false;
                    game.util.restartGame(game.state.Gameplay);
                }
            }, 100);
            me.timer.setTimeout(()=>{
                this.click = true;
