...
function Application()
{
	...
	*/
	...
	var _BGCOLOR = "#ffffff";
	var _FONT = "Arial";
	var _TITLE = "Cheerful Plumber Coloring";
	...
	*/
	this.copyright = "© DL-Studio, v 5.0";
	/*
	...
	*/
	function _resetFunc()
	{
		...
		*/
		_information.currentHexColor = fromDecimalToHexColorFunc(_information.currentColor);
		/*
	...
	function _selectFunc()
	{
		...
		*/
		if(_currentMainScreen_mc.moreGames_btn)
		{
			...
			//createjs.Tween.get(_currentMainScreen_mc.moreGames_btn).wait(_DELAY * 1000).to({alpha:1}, _DELAY * 1000, createjs.Ease.cubicInOut);
			_currentMainScreen_mc.moreGames_btn.addEventListener("click", _onClickMoreGamesBtnFunc, false);
		}
		/*
	...
	*/
	function _setContainerSelectFunc()
	{
		if(_currentMainScreen_mc.locations_mc)
		{
			...
			*/
			_currentMainScreen_mc.locations_mc.gotoAndStop(0);
			/*
			
			*/
			if(_currentMainScreen_mc.locations_mc.totalFrames > 1)
			{
				if(_hasLabelInMovieClipFunc(_currentMainScreen_mc.locations_mc, "mid"))
				{
					_currentMainScreen_mc.locations_mc.loop = false;
					var timelineControl = new Edapskov_TimelineControl(_currentMainScreen_mc.locations_mc);
		...
		*/
		if(_currentMainScreen_mc.locations_mc)
		{
			for(var propLocations in _information.locations)
			{
				if(_information.locations.hasOwnProperty(propLocations))
				{
					if(_currentMainScreen_mc.locations_mc[propLocations])
					{
						/*
						
						*/
						location_mc = _currentMainScreen_mc.locations_mc[propLocations];
						location_mc.gotoAndStop(0);
						location_mc.name = propLocations;
						...
						*/
						if(_currentMainScreen_mc.locations_mc[propLocations].completed)
						{
	...
	*/
	...
	function _onClickLocationSelectFunc(event)
	{
		...
		*/
		//new Edapskov_Flickering(_before, location_mc, ["#ffffff", "#ffffcc"], 10, 16);
		new Edapskov_Flickering(_before, location_mc, ["#ffffff", "#ffffcc"], 8, 90, 1, "Glitter_2");
		/*
		...
		*/
		_currentMainScreen_mc.locations_mc.mouseEnabled = false;
		_currentMainScreen_mc.locations_mc.mouseChildren = false;
		/*
		
		*/
		if(_hasLabelInMovieClipFunc(_currentMainScreen_mc.locations_mc, "end"))
		{
			var timelineControl = new Edapskov_TimelineControl(_currentMainScreen_mc.locations_mc);
	...
	*/
	function _locationFunc()
	{
		...
		*/
		if(_currentMainScreen_mc.back_btn)
		{
			_currentMainScreen_mc.back_btn.alpha = 0.01;
			createjs.Tween.get(_currentMainScreen_mc.back_btn).wait((_DELAY * 1.5) * 1000).to({alpha:1}, _DELAY * 1000, createjs.Ease.cubicInOut);
			_currentMainScreen_mc.back_btn.addEventListener("click", _onClickBackBtnLocationFunc, false);
		}
		/*
		...
		*/
		...
		{
			_currentMainScreen_mc.photo_btn.alpha = 0.01;
			createjs.Tween.get(_currentMainScreen_mc.photo_btn).wait((_DELAY * 1.0) * 1000).to({alpha:1}, _DELAY * 1000, createjs.Ease.cubicInOut);
			_currentMainScreen_mc.photo_btn.addEventListener("click", _onClickPhotoBtnLocationFunc, false);
		}
		/*
		
		*/
		_beginGameLocationFunc();
		/*
	...
	*/
	function _beginGameLocationFunc()
	{
		/*
		исключение
		*/
		if(_currentMainScreen_mc.photoDecor_mc)
		{
			_currentMainScreen_mc.photoDecor_mc.mouseChildren = false;
			_currentMainScreen_mc.photoDecor_mc.mouseEnabled = false;
			_currentMainScreen_mc.photoDecor_mc.visible = false;
		}
		/*
		...
		*/
		_updateModeLocationFunc();
		/*
		
		*/
		_currentMainScreen_mc.panel_mc.colorBar_mc.gotoAndStop(0);
		_currentMainScreen_mc.panel_mc.colorBar_mc.mouseEnabled = true;
		_currentMainScreen_mc.panel_mc.colorBar_mc.mouseChildren = true;
		_currentMainScreen_mc.panel_mc.colorBar_mc.cursor = "crosshair";
		_currentMainScreen_mc.panel_mc.colorBar_mc.addEventListener("click", _onClickColorBarMcLocationFunc, false);
		/*
		
		*/
		if(_currentMainScreen_mc.panel_mc.palette_mc)
		{
			/*
			
			*/
			_currentMainScreen_mc.panel_mc.palette_mc.gotoAndStop(0);
			_currentMainScreen_mc.panel_mc.palette_mc.cursor = "crosshair";
			_currentMainScreen_mc.panel_mc.palette_mc.visible = false;
			_currentMainScreen_mc.panel_mc.palette_mc.mouseEnabled = false;
			_currentMainScreen_mc.panel_mc.palette_mc.mouseChildren = false;
			/*
			
			*/
			if(_currentMainScreen_mc.panel_mc.openPalette_btn)
			{
				_currentMainScreen_mc.panel_mc.openPalette_btn.addEventListener("click", _onClickOpenPaletteBtnLocationFunc, false);
			}
			else
			{
				trace("А ты уверен, что тебе не нужна openPalette_btn ?");
			}
		}
		/*
		...
		*/
		_currentMainScreen_mc.photoContainer_mc.decorTop_mc.gotoAndStop(0);
		_currentMainScreen_mc.photoContainer_mc.decorTop_mc.mouseChildren = false;
		_currentMainScreen_mc.photoContainer_mc.decorTop_mc.mouseEnabled = false;
		/*
		
		*/
		_updatePartsLocationFunc();
		/*
		
		*/
		_currentMainScreen_mc.photoContainer_mc.decorBottom_mc.gotoAndStop(0);
		_currentMainScreen_mc.photoContainer_mc.decorBottom_mc.mouseChildren = false;
		_currentMainScreen_mc.photoContainer_mc.decorBottom_mc.mouseEnabled = false;
	}
	/*
	
	*/
	function _updatePartsLocationFunc()
	{
		/*
		
		*/
		_currentMainScreen_mc.photoContainer_mc.parts_mc.gotoAndStop(0);
		var totalParts_num = _currentMainScreen_mc.photoContainer_mc.parts_mc.numChildren;
		var i = 0;
		var part_mc;
		var figure_mc;
		var figure_shape;
		while(i < totalParts_num)
		{
			/*
			
			*/
			part_mc = _currentMainScreen_mc.photoContainer_mc.parts_mc.getChildAt(i);
			/*
			
			*/
			if(part_mc.totalFrames)
			{
				/*
				
				*/
				part_mc.gotoAndStop(0);
				/*
				
				*/
				if(part_mc.figure_mc)
				{
					/*
					
					*/
					part_mc.name = "part_" + i + "_mc";
					part_mc.cursor = "crosshair";
					part_mc.mouseChildren = false;
					part_mc.mouseEnabled = true;
					part_mc.addEventListener("mousedown", _onMouseDownPartMcLocationFunc, false);
					/*
					
					*/
					figure_mc = part_mc.figure_mc;
					figure_mc.gotoAndStop(0);
					figure_mc.x = 0;
					figure_mc.y = 0;
					/*
					
					*/
					_clearContainerFunc(figure_mc);
					/*
					
					*/
					cacheFigure_cont = new createjs.Container();
					figure_mc.addChildAt(cacheFigure_cont, 0);
					cacheFigure_cont.name = cacheFigure_cont;
					/*
					
					*/
					canvasFigure_cont = new createjs.Container();
					figure_mc.addChildAt(canvasFigure_cont, 1);
					canvasFigure_cont.name = canvasFigure_cont;
					/*
					
					*/
					figure_shape = new createjs.Shape();
					canvasFigure_cont.addChildAt(figure_shape, 0);
				}
			}
			/*
	...
	function _onMouseDownPartMcLocationFunc(event)
	{
		/*
		
		*/
		var currentPart_mc = event.currentTarget;
		_information.currentPart = currentPart_mc;
		/*
		
		*/
		var figure_mc = currentPart_mc.figure_mc;
		var cacheFigure_cont = figure_mc.getChildAt(0);
		var canvasFigure_cont = figure_mc.getChildAt(1);
		/*
		
		*/
		var figure_shape = canvasFigure_cont.getChildAt(0);
		/*
		
		*/
		var point_obj = currentPart_mc.localToLocal(event.localX, event.localY, figure_shape);
		/*
		
		*/
		if(_information.currentMode >= 1 && _information.currentMode <= 5)
		{
			/*
			
			*/
			figure_shape.graphics.beginFill(_information.currentHexColor);
			figure_shape.graphics.drawCircle(point_obj.x, point_obj.y, _getLineThicknessLocationFunc(_information.currentMode) / 2);
			figure_shape.graphics.endFill();
			/*
			
			*/
			figure_shape.graphics.setStrokeStyle(_getLineThicknessLocationFunc(_information.currentMode), "round");
			figure_shape.graphics.beginStroke(_information.currentHexColor);
			figure_shape.graphics.moveTo(point_obj.x, point_obj.y);
			/*
			
			*/
			_stage.addEventListener("stagemousemove", _onStageMouseMovePartMcLocationFunc, false);
			_stage.addEventListener("stagemouseup", _onStageMouseUpPartMcLocationFunc, false);
		}
		else if(_information.currentMode == 6)
		{
			try
			{
				/*
				...
				*/
				_currentMainScreen_mc.cache(0, 0, _CWIDTH, _CHEIGHT);
				/*
				
				*/
				var bitmapdata_obj = createjs.BitmapData.getBitmapData(_currentMainScreen_mc);
				point_obj = currentPart_mc.localToLocal(event.localX, event.localY, _currentMainScreen_mc);
				/*
				
				*/
				_information.currentColor = bitmapdata_obj.getPixel(point_obj.x, point_obj.y);
				_information.currentHexColor = fromDecimalToHexColorFunc(_information.currentColor);
				/*
				
				*/
				_information.currentMode = _information.lastBrush;
				/*
		...
		else
		{
			...
			*/
			figure_shape.graphics.clear();
			/*
			
			*/
			_clearContainerFunc(cacheFigure_cont);
		}
	}
	function _onStageMouseMovePartMcLocationFunc(event)
	{
		/*
		
		*/
		var currentPart_mc = _information.currentPart;
		/*
		...
		*/
		var point_obj = event.currentTarget.localToLocal(event.localX, event.localY, figure_shape);
		/*
		
		*/
		if(currentPart_mc.body_mc.hitTest(point_obj.x, point_obj.y))
		{
			/*
			
			*/
			figure_shape.graphics.lineTo(point_obj.x, point_obj.y);
			figure_shape.graphics.endStroke();
			/*
			
			*/
			_information.counterIntervals++;
			if(_information.counterIntervals > 20 && createjs.Ticker.framerate < 10)
			{
				/*
				
				*/
				_information.counterIntervals = 0;
				/*
				
				*/
				_updateCacheCurrentFigureMcLocationFunc(figure_mc);
			}
			/*
	...
	function _onStageMouseUpPartMcLocationFunc(event)
	{
		/*
		...
		*/
		_stage.removeEventListener("stagemousemove", _onStageMouseMovePartMcLocationFunc, false);
		_stage.removeEventListener("stagemouseup", _onStageMouseUpPartMcLocationFunc, false);
		/*
		
		*/
		_updateCacheCurrentFigureMcLocationFunc(figure_mc);
	}
	function _updateCacheCurrentFigureMcLocationFunc(figure_mc)
	{
		if(figure_mc !== undefined)
		{
			/*
			...
			*/
			try
			{
				...
				*/
				var bounds_rect = figure_mc.getBounds();
				figure_mc.cache(bounds_rect.x, bounds_rect.y, bounds_rect.width, bounds_rect.height);
				/*
				
				*/
				var figure_img = new Image();
				figure_img.src = figure_mc.getCacheDataURL();
				var figure_bitmap = new createjs.Bitmap(figure_img);
				_clearContainerFunc(cacheFigure_cont);
				cacheFigure_cont.addChildAt(figure_bitmap, 0);
				figure_bitmap.x = bounds_rect.x;
				figure_bitmap.y = bounds_rect.y;
				/*
	...
	function _getLineThicknessLocationFunc(currentMode_uint)
	{
		var thickness_num = 10;
		if(currentMode_uint == 1)
		{
			thickness_num = 10;
		}
		else if(currentMode_uint == 2)
		{
			thickness_num = 20;
		}
		else if(currentMode_uint == 3)
		{
			thickness_num = 40;
		}
		else if(currentMode_uint == 4)
		{
			thickness_num = 60;
		}
		else
	...
	*/
	function _updateModeLocationFunc()
	{
		/*
		
		*/
		var drawingModes_mc = _currentMainScreen_mc.panel_mc.drawingModes_mc;
		drawingModes_mc.gotoAndStop(0);
		var mode_mc;
		var i = 0;
		while(i < _information.totalModes)
		{
			/*
			
			*/
			if(drawingModes_mc["mode_" + i + "_mc"])
			{
				/*
				
				*/
				mode_mc = drawingModes_mc["mode_" + i + "_mc"];
				mode_mc.gotoAndStop(0);
				mode_mc.name = "mode_" + i + "_mc";
				/*
				
				*/
				if(i == _information.currentMode)
				{
					/*
					
					*/
					mode_mc.mouseEnabled = false;
					mode_mc.mouseChildren = false;
					mode_mc.cursor = "default";
					mode_mc.removeEventListener("click", _onClickModeMcLocationFunc, false);
					/*
					
					*/
					if(mode_mc.check_mc)
					{
						mode_mc.check_mc.gotoAndStop(1);
					}
				}
				else
				{
					...
					*/
					mode_mc.mouseEnabled = true;
					mode_mc.mouseChildren = true;
					mode_mc.cursor = "pointer";
					mode_mc.addEventListener("click", _onClickModeMcLocationFunc, false);
					/*
					
					*/
					if(mode_mc.check_mc)
					{
						mode_mc.check_mc.gotoAndStop(0);
					}
				}
				/*
				...
				*/
				if(mode_mc.color_mc)
				{
					_colouringFunc(mode_mc.color_mc, _information.currentColor);
				}
				/*
				
				*/
				if(mode_mc.animation_mc)
				{
					if(mode_mc.animation_mc.object_mc)
					{
						if(mode_mc.animation_mc.object_mc.color_mc)
						{
							_colouringFunc(mode_mc.animation_mc.object_mc.color_mc, _information.currentColor);
						}
					}
				}
			}
	...
	*/
	function _onClickModeMcLocationFunc(event)
	{
		/*
		
		*/
		var currentMode_mc = event.currentTarget;
		/*
		
		*/
		var point_obj = currentMode_mc.localToLocal(event.localX, event.localY, _stage);
		_soundManager.addSoundFunc("click_sound", 0, 0, 0, 1, _soundManager.panFromStageFunc(point_obj.x, _CWIDTH));
		/*
		
		*/
		_information.currentMode = parseInt(currentMode_mc.name.split("_")[1]);
		/*
		
		*/
		if(_information.currentMode >= 1 && _information.currentMode <= 5)
		{
			_information.lastBrush = _information.currentMode;
		}
		/*
	...
	*/
	function _onClickOpenPaletteBtnLocationFunc(event)
	{
		/*
		...
		*/
		_togglePaletteLocationFunc();
	}
	function _togglePaletteLocationFunc()
	{
		if(_currentMainScreen_mc.panel_mc.palette_mc.visible)
		{
			_currentMainScreen_mc.panel_mc.palette_mc.visible = false;
			_currentMainScreen_mc.panel_mc.palette_mc.mouseEnabled = false;
			_currentMainScreen_mc.panel_mc.palette_mc.mouseChildren = false;
			_currentMainScreen_mc.panel_mc.palette_mc.removeEventListener("click", _onClickPaletteMcLocationFunc, false);
		}
		else
		{
			_currentMainScreen_mc.panel_mc.palette_mc.visible = true;
			_currentMainScreen_mc.panel_mc.palette_mc.mouseEnabled = true;
			_currentMainScreen_mc.panel_mc.palette_mc.mouseChildren = true;
			_currentMainScreen_mc.panel_mc.palette_mc.addEventListener("click", _onClickPaletteMcLocationFunc, false);
		}
	}
	function _onClickColorBarMcLocationFunc(event)
	{
		_updateCurrentColorLocationFunc(event.currentTarget, event.localX, event.localY);
	}
	function _onClickPaletteMcLocationFunc(event)
	{
		/*
	...
	function _updateCurrentColorLocationFunc(container_mc, coordX_num, coordY_num)
	{
		/*
		
		*/
		var colors_img = container_mc.getChildAt(0);
		var clone_img = colors_img.clone();
		var bounds_rect = clone_img.getBounds();
		clone_img.cache(bounds_rect.x, bounds_rect.y, bounds_rect.width, bounds_rect.height);
		var bitmapdata_obj = createjs.BitmapData.getBitmapData(clone_img);
		/*
		
		*/
		_information.currentColor = bitmapdata_obj.getPixel(coordX_num, coordY_num);
		_information.currentHexColor = fromDecimalToHexColorFunc(_information.currentColor);
		/*
	...
	*/
	function _onClickBackBtnLocationFunc(event)
	{
		/*
		...
		*/
		event.currentTarget.removeEventListener("click", _onClickBackBtnLocationFunc, false);
		/*
		
		*/
		_information.counterAds++;
		if(_information.counterAds == 2)
		{
	...
	*/
	function _onClickPhotoBtnLocationFunc(event)
	{
		/*
		...
		*/
		if(_currentMainScreen_mc.photoDecor_mc)
		{
			_currentMainScreen_mc.photoDecor_mc.visible = true;
		}
		/*
	...
	*/
	function fromDecimalToHexColorFunc(decimal_num)
	{
		var hex_str = Number(decimal_num).toString(16);
		hex_str = "000000".substr(0, 6 - hex_str.length) + hex_str;
		return "#" + hex_str;
	}
	/*
	==============================================================================================================
