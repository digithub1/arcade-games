...
function share(n){
  if(gp!=null) gp.socials.share({
    text: lang == 'ru' ? 'Скулбой VS Бати: Линия Фронта — командный экшен с захватом точек, крутыми картами, оружием, скинами и бустерами. Сражайся против Бати!' : 'Schoolboy vs Daddy: Frontline — a team-based action game with point capture, cool maps, weapons, skins, and boosters. Fight against Dad!'
  });
}
...
function buyMicrosxeme() {
  ...
  ysdk.adv.showRewardedVideo({
    callbacks: {
      onClose: function(secx) {
        advscr = false;
        if(secx)unityInstance.SendMessage('InAppSys', 'MicrosxemeRecive');
		    unityInstance.SendMessage('Interop', 'PostBanner');
      }
    }
  });
...
function yarbanner() {
  ...
  ysdk.adv.showRewardedVideo({
    callbacks: {
      onClose: function(secx) {
        advscr = false;
        if(secx)unityInstance.SendMessage('Interop', 'REWARDCOMPLITE');
		    unityInstance.SendMessage('Interop', 'PostBanner');
      }
    }
  });
