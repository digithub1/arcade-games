...
game.object.HomeroomDragArea = me.Sprite.extend({
	init: function(x, y, settings = {}){
		settings.texture = "Cafetaria";
    ...
    dragStart: function (e) {
        if(this.dragging === false && e.gameX > 827 && e.gameX < 1553 && e.gameY > 200 && e.gameY < 500) {
            this.dragging = true;
            this.touchPosY = e.gameY;
            
            if(game.user.userData.homeroomTutorial == 0)
                this.container.Dialog('tutorial');
            return false;
        }
        else if(this.dragging == false && game.user.userData.homeroomTutorial == 0){
            this.dragging = true;
            this.container.Dialog('tutorial');
            return false;
        }
    ...
    dragMove: function (e) {
        if (this.dragging === true) {
            let moveY = e.gameY - this.touchPosY;
            if(e.gameY < 10){
                this.dragEnd();
                return false;
            }
            
            if(e.gameY > me.game.viewport.height - 10){
                this.dragEnd();
                return false;
            }
            ...
            if(this.container.panelList['HR000']['panel'].pos.y + moveY < this.container.limitAtas){
                moveY = this.container.limitAtas - this.container.panelList['HR000']['panel'].pos.y;
            }
                
            if(this.container.panelList['HR000']['panel'].pos.y + moveY > this.container.limitBawah){
                moveY = this.container.limitBawah - this.container.panelList['HR000']['panel'].pos.y;
            }
            
            for(let i in this.container.panelList){
                for(let j in this.container.panelList[i]){
                    this.container.panelList[i][j].pos.y += moveY;
                }
            }
