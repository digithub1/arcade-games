...
game.object.LibraryHeroBtn = me.Sprite.extend({
	init: function(x, y, settings = {}){
		...
		settings.frameheight = settings.frameheight || 127;
    ...
    // Put user code here //
    Show : function(){
        game.var.libraryData["category"] = "hero";
        this.container.textName.setText(" ");
        this.container.textDesc.setText(" ");
        this.container.HideEnemy();
        this.btnSprite.setCurrentAnimation('idle');
        this.container.enemyBtnSprite.setCurrentAnimation('disable');
            
        for(let i in this.container.enemyBookList){
            this.container.enemyBookList[i].pos.x = this.container.enemyBookList[i].startPosX;
            this.container.enemyBookList[i].alpha = 0;
        }
            
        for(let i in this.container.heroBookList){
            this.container.heroBookList[i].alpha = 1;
        }
            
        this.container.bookList = this.container.heroBookList;
