...
function Application()
{
	...
	*/
	...
	var _BGCOLOR = "#ffffff";
	var _FONT = "Arial";
	var _TITLE = "Smile Style";
	var _LINKTOGAME = "https://dl-girls.com";
	var _DELAY = 1;
	var _LANGUAGE = "en";
	/*
	публичные переменные
	специфическая версия под специфические цели
	*/
	...
	*/
	function _onClickPlayBtnFunc(event)
	{
		...
		*/
		...
		исключение
		*/
		_information.preview_mc = null;
		if(_skippedLocationsFunc() <= 0)
		{
			_resetFunc();
		}
		/*
		...
		if(currentScreen_mc.locations_mc)
		{
			...
			*/
			for(var propLocations in _information.locations)
			{
				if(_information.locations.hasOwnProperty(propLocations))
				{
					if(locations_mc[propLocations])
					{
						/*
						
						*/
						location_mc = locations_mc[propLocations];
						location_mc.gotoAndStop(0);
						location_mc.name = propLocations;
						/*
						
						*/
						if(_information.locations[propLocations].completed)
						{
							/*
							location_mc.gotoAndStop(0);
	...
	*/
	...
	function _endActionsFunc()
	{
		...
		*/
		_information.locations[_information.currentNameLocation].currentAction = 1;
		_information.locations[_information.currentNameLocation].currentNameAction = "action_" + _information.locations[_information.currentNameLocation].currentAction;
		/*
	...
	*/
	function _onClickPlayRedirectBtnFunc(event)
	{
		...
		*/
		window.open("https://dl-girls.com/content/folder_1669050747" + "?utm_source=" + window.location.href + "&utm_medium=game_referral|game&utm_content=simple&utm_campaign=" + _TITLE, "_self");
	}
	/*
	...
	*/
	function _resultFunc()
	{
		...
		*/
		...
		if(_hasLabelInMovieClipFunc(currentScreen_mc.photoContainer_mc.heroes_mc, _information.currentNameLocation))
		{
			/*
			
			*/
			currentScreen_mc.photoContainer_mc.heroes_mc.gotoAndStop(_information.currentNameLocation);
			/*
			
			*/
			if(currentScreen_mc.photoContainer_mc.heroes_mc[_information.currentNameLocation])
			{
				/*
				
				*/
				_information.preview_mc = currentScreen_mc.photoContainer_mc.heroes_mc[_information.currentNameLocation];
				_information.preview_mc.gotoAndStop(0);
				/*
				
				*/
				if(_skippedLocationsFunc() > 0)
				{
					_information.preview_mc.gotoAndStop(1);
				}
			}
		}
		/*
	...
	*/
	...
	function _onClickPrevNavigationMcBackgroundsFunc(event)
	{
		...
		*/
		var currentScreen_mc = _main.getChildAt(0);
		var totalFrames_num = _information.preview_mc.totalFrames;
		var currentFrame_num = _information.preview_mc.currentFrame + 1;
		currentFrame_num--;
		if(_skippedLocationsFunc() > 0)
		{
			if(currentFrame_num <= 1)
			{
				currentFrame_num = totalFrames_num;
			}
		}
		...
		*/
		_information.preview_mc.gotoAndStop(Number(currentFrame_num - 1));
	}
	...
	*/
	...
	{
		window.open("https://dl-girls.com/index.php?controller=article&action=index&id=3552" + "&utm_source=" + window.location.href + "&utm_medium=game_referral|game&utm_content=simple&utm_campaign=" + _TITLE, "_blank");
	}
	function _onClickBannerMcFunc_2(event)
	{
		window.open("https://dl-girls.com/index.php?controller=article&action=index&id=3577" + "&utm_source=" + window.location.href + "&utm_medium=game_referral|game&utm_content=simple&utm_campaign=" + _TITLE, "_blank");
	}
